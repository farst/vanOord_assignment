...
def create_large_dataset(num_records=1000000):
    ...
    for i in range(num_records):
        ...
        order_date = base_date + timedelta(days=random.randint(0, 1460))  # 4 years of data
...
for query_name, query_template in queries.items():
    ...
    unopt_time = run_performance_test(unopt_query, "unoptimized_orders", "Unoptimized Table")
    
    # Test partitioned table
    part_query = query_template.format("partitioned_orders")
    part_time = run_performance_test(part_query, "partitioned_orders", "Partitioned Table")
    ...
    print(f"Performance improvement: {performance_results[query_name]['improvement']:.1f}%")
...
spark.sql(f"OPTIMIZE delta.`{zordered_path}` ZORDER BY (customer_id, product_id, region)")
...
def apply_advanced_optimizations():
    ...
    try:
        spark.sql(f"ALTER TABLE zordered_orders SET TBLPROPERTIES ('delta.bloomFilter.customer_id' = 'true')")
        spark.sql(f"ALTER TABLE zordered_orders SET TBLPROPERTIES ('delta.bloomFilter.product_id' = 'true')")
    except:
    ...
    spark.sql(f"ALTER TABLE zordered_orders SET TBLPROPERTIES ('delta.autoOptimize.optimizeWrite' = 'true')")
    spark.sql(f"ALTER TABLE zordered_orders SET TBLPROPERTIES ('delta.autoOptimize.autoCompact' = 'true')")
...
def create_performance_dashboard():
    ...
    SELECT 
        ...
        SUM(sizeInBytes) as total_size_mb
    FROM delta.`{}`
    """.format(unoptimized_path, partitioned_path, zordered_path)
...
def demonstrate_advanced_patterns():
    ...
    dim_customers = spark.createDataFrame([
        {"customer_id": "CUST00001", "customer_tier": "Premium", "join_date": "2020-01-01"},
        {"customer_id": "CUST00002", "customer_tier": "Standard", "join_date": "2020-02-01"},
        {"customer_id": "CUST00003", "customer_tier": "Premium", "join_date": "2020-03-01"},
    ])
