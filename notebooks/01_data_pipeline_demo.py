...
df_cleaned = (
    df_csv.withColumn("amount", col("amount").cast("double"))
...
df_cleaned.write.format("delta").mode("overwrite").partitionBy("year", "month").save(
    delta_path
)
...
df_new_cleaned.write.format("delta").mode("append").save(delta_path)
...
spark.sql(
    "SELECT * FROM delta.`{}` VERSION AS OF 0 ORDER BY date DESC".format(delta_path)
).show()

# Show version differences
print("Version comparison:")
current_count = spark.sql(
    "SELECT COUNT(*) as current_count FROM invoices_delta"
).collect()[0]["current_count"]
previous_count = spark.sql(
    "SELECT COUNT(*) as previous_count FROM delta.`{}` VERSION AS OF 0".format(
        delta_path
    )
).collect()[0]["previous_count"]
