...
def create_ge_expectation_suite():
    ...
    print(
        f"Great Expectations suite '{suite_name}' created with {len(expectations)} expectations"
    )
    return suite_name
...
def run_ge_validation():
    ...
    # Run validation
    results = df_ge.validate(expectation_suite=suite, result_format="SUMMARY")
...
def create_deequ_verification():
    ...
    # Add checks
    check = (
        check.hasSize(lambda x: x >= 10, "Dataset has at least 10 records")
    ...
    # Run verification
    verification_result = (
        VerificationSuite(spark).onData(df_sample).addCheck(check).run()
    )
...
def run_anomaly_detection():
    ...
    check = check.hasAnomalyDetection(
        "age",
        AnomalyCheckConfig(
...
def create_quality_dashboard():
    ...
    valid_records = df_sample.filter(
        col("customer_id").isNotNull() & col("email").isNotNull() & col("age")
        > 0 & col("age")
        <= 120 & col("salary")
        > 0 & col("city").isNotNull()
    ).count()
    ...
    issues = {
        ...
        "Invalid Email Format": df_sample.filter(
    ...
    print(
        f"     Min: {age_stats['min_age']}, Max: {age_stats['max_age']}, Avg: {age_stats['avg_age']:.1f}"
    )

    # Salary distribution
    print("   Salary Distribution:")
    salary_stats = (
        df_sample.filter(col("salary") > 0)
        .select(
    ...
    print(
        f"     Min: ${salary_stats['min_salary']:,.0f}, Max: ${salary_stats['max_salary']:,.0f}, Avg: ${salary_stats['avg_salary']:,.0f}"
    )
...
def implement_quality_gates():
    ...
    def check_quality_gates(df):
        ...
        for column in quality_thresholds["required_columns"]:
            if column in df.columns:
                ...
                completeness_checks[column] = (
                    null_percentage <= quality_thresholds["max_null_percentage"]
                )
        ...
        if "customer_id" in df.columns:
            ...
            results["duplicates"] = (
                duplicate_percentage <= quality_thresholds["max_duplicate_percentage"]
            )
        else:
        ...
        results["quality_score"] = (
            quality_score >= quality_thresholds["min_quality_score"]
        )
    ...
    print("=" * 40)
    print(
        f"Required Columns Present: {'✅' if gate_results['missing_columns'] else '❌'}"
    )
    print(f"Data Completeness: {'✅' if gate_results['completeness'] else '❌'}")
    print(f"Duplicate Check: {'✅' if gate_results['duplicates'] else '❌'}")
    print(
        f"Quality Score ({quality_score:.1f}%): {'✅' if gate_results['quality_score'] else '❌'}"
    )
...
def integrate_quality_with_delta():
    ...
    df_with_quality = (
        df_sample.withColumn("quality_score", lit(quality_score))
    ...
    df_with_quality.write.format("delta").mode("overwrite").save(quality_table_path)
    ...
    def validate_data_quality(table_path, min_quality_score=80.0):
        ...
        df_updated = (
            df.withColumn("quality_score", lit(current_quality_score))
            .withColumn("quality_timestamp", current_timestamp())
            .withColumn(
                "quality_gate_passed", lit(current_quality_score >= min_quality_score)
            )
        )
